...
const { getDropPoint, getBestBody, smartMove, announce, acquireEnergy } = require('./helpers');
...
const { manageExpansionPopulation, getEnabledExpansionRooms, isRoomOwned, getExpansionMemory, runExpansionRoles } = require('./expansion');
...
module.exports.loop = function () {
    ...
    for (let name in Game.creeps) {
        ...
        if (role === 'scout' || role === 'claimer' || role === 'expansionMiner' || role === 'expansionHauler') {
            runExpansionRoles(creep);
        } else if (roles[role]) {
            roles[role](creep, roomMem);
        }
    }
