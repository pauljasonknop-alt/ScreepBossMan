...
module.exports.loop = function () {
    ...
    // Run infrastructure
    autoBuild(room);
    runTowers(room); // Now runs EVERY tick for maximum responsiveness
    
    // EMERGENCY DEFENSE - If enemies detected and we have no towers/fighters, spawn emergency fighter
let enemies = room.find(FIND_HOSTILE_CREEPS);
if (enemies.length > 0) {
...
if (Game.time % 50 === 0) {
    ...
    if (mineral && mineral.mineralAmount > 0) {
        console.log(`\n⛏️  MINERAL: ${mineral.mineralType} | ${mineral.mineralAmount}/${mineral.mineralCapacity} (${Math.floor(mineral.mineralAmount / mineral.mineralCapacity * 100)}%)`);
    ...
    if (towers.length > 0) {
        ...
        for (let tower of towers) {
            ...
            console.log(`   Tower at (${tower.pos.x},${tower.pos.y}): ⚡ ${energyPercent}%`);
        }
    }
